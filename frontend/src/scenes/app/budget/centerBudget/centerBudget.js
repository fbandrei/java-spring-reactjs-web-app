...
class CenterBudget extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      time.year !== this.state.currentTime.year ||
    ...
    if (input.value !== this.state.currentInputValue) {
      ...
      if (id[2] === "s") {
        if (id[4] !== undefined) {
          ...
          updateToBeBudget(
            Number(this.props.toBeBudget) -
              Number(input.value) +
              Number(budget.budget)
          ).then(
            this.props.setToBeBudget(
              Number(this.props.toBeBudget) -
                Number(input.value) +
                Number(budget.budget)
            )
        ...
        if (id[5] !== undefined) {
          subcategory =
            data[id[1] + "" + id[2]].subcategories[id[4] + "" + id[5]];
          let budget = subcategory.budgets[0];
          updateToBeBudget(
            Number(this.props.toBeBudget) -
              Number(input.value) +
  ...
  focusElement() {
    this.setState({
      ...
      currentInputValue: document.getElementById(document.activeElement.id)
        .value
    });
  }
  ...
  render() {
    ...
    if (arrayData !== undefined) {
      for (let i = 0; i < arrayData.length; i++) {
        const category = {
          key: i + 1,
          category: arrayData[i].name,
          budget: this.state.budget[i].budgeted,
          activity: this.state.budget[i].activity,
          ...
          delete: (
            <div>
              <Button
                icon={"delete"}
                type={"danger"}
                onClick={this.handleVisibleChangeCategory}
                id={i + ""}
              />
              <Popover
                id={"" + i}
              ...
              >
                <div>
                  ...
                  <span>
                    <Button
                      onClick={this.hideDeleteCategoryPopover}
                      className={"deleteCategoryPopoverNoButton"}
                    >
                      No
                    </Button>
                    <Button
                      ...
                      type={"danger"}
                      className={"deleteCategoryPopoverYesButton"}
                    >
                      Yes{" "}
                    </Button>
                  </span>
      ...
      if (category !== undefined && category.subcategories !== undefined) {
        for (let i = 0; i < category.subcategories.length; i++) {
          if (category.subcategories[i].budgets[zero] !== undefined) {
            ...
            const m = {
              key: i,
              category: category.subcategories[i].name,
              budget: (
                <InputNumber
                  defaultValue={category.subcategories[i].budgets[zero].budget}
                  min={0}
                  max={9999999}
                  step={0.25}
                  size={"small"}
                  onBlur={this.submitBudget.bind(this)}
                  onFocus={this.focusElement.bind(this)}
                  id={id}
                />
              ),
              ...
              available: (
                <Button size={"small"}>
                  {category.subcategories[i].budgets[zero].availableAmount}
                </Button>
              ),
              delete: (
                <div>
                  <Button
                    icon={"delete"}
                    type={"danger"}
                    onClick={this.handleVisibleChangeSubcategory}
                    id={id2}
                  />
                  <Popover
                    id={"" + i}
                  ...
                  >
                    <div>
                      ...
                      <span>
                        <Button
                          onClick={this.hideDeleteSubcategoryPopover}
                          className={"deleteCategoryPopoverNoButton"}
                        >
                          No
                        </Button>
